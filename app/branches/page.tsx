        ...
        <motion.div key={category} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-center">
            {activeBranches.map((branch, index) => (
              <motion.div key={branch.slug} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="glass-card bg-black/40 border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col group hover:border-white/20">
                <div className="w-full h-48 sm:h-56 overflow-hidden relative bg-[#0a0a0a]">
                  <img src={branch.image} alt={branch.name} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-700" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                </div>
                <div className="p-6 sm:p-8 flex-grow flex flex-col">
                  <h2 className="text-2xl font-black text-white mb-6">{branch.name}</h2>
                  <div className="space-y-4 mb-8 flex-grow">
                    <div className="flex items-center gap-3 text-gray-300"><MapPin className="text-[#FFCC00]" size={20} /><span className="text-sm font-medium">{branch.address}</span></div>
                    <div className="flex items-center gap-3 text-gray-300"><Clock className="text-[#FFCC00]" size={20} /><span className="text-sm font-medium">{branch.hours}</span></div>
                    <div className="flex items-center gap-3 text-gray-300"><Phone className="text-[#FFCC00]" size={20} /><span className="text-sm font-medium">{branch.phone}</span></div>
                  </div>
                  {/* Note: Update the link destination depending on if you combine the details pages or keep them separate! */}
                  <Link 
