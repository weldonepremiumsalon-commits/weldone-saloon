...
import { MEN_TEAM, WOMEN_TEAM } from "@/lib/data";

export default function TeamPage() {
  const { category } = useCategory();
  const activeTeam = category === "women" ? WOMEN_TEAM : MEN_TEAM;
  const title = category === "women" ? "STYLISTS" : "BARBERS";
  ...
  return (
    <div className="relative min-h-screen pt-40 sm:pt-48 pb-24 px-4 sm:px-6 overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={category} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="max-w-7xl mx-auto relative z-10">
